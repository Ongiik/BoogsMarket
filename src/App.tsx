...
const App = () => {
  ...
  const handleAddToCart = (clickedItem: CartItemType) => {
    setCartItems(prev => {
      //1. is the item alr in cart?
      const isItemInCart = prev.find(item => item.id === clickedItem.id) //check if item exists in previous state

      if (isItemInCart) {
        //can update the amount by mapping through the items.
        return prev.map(item => (
          item.id === clickedItem.id
            ? { ...item, amount: item.amount + 1} //if item found
            : item 
        ))
      }
